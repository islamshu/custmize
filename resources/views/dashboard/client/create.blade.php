                    ...
                    <div class="row breadcrumbs-top">
                        <div class="breadcrumb-wrapper col-12">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active">{{ __('Add Client') }}
                                </li>
                            </ol>
                        </div>
                    </div>
                        ...
                        <div class="col-12">
                            <div class="card">
                                ...
                                <div class="card-content collpase show">
                                    <div class="card-body">
                                        <input type="hidden" value="{{ route('clients.store') }}" id="url_route">
                                        <form class="form" id="add_full_Client">
                                            @csrf
                                            <div class="form-body">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="form-group col-md-6 mb-2">
                                                    ...
                                                    <input
                                                        ...
                                                        class="form-control border-primary" @if($item->required_or_not == 'required') required @endif
                                                        placeholder="{{ __(str_replace('_',' ',$item->name)) }}" name="extra[{{ $item->name }}]">
                                                        @else
                                                        <textarea @if($item->required_or_not == 'required') required @endif name="extra[{{ $item->name }}]" class="form-control"  placeholder="{{ __(str_replace('_',' ',$item->name)) }}"></textarea>
                                                        @endif
...
@section('script')
    <script>
        ...
        $("#add_full_Client").submit(function(event) {
            ...
            $.ajax({
                type: 'post',
                url: url,
                data: new FormData($('#add_full_Client')[0]),
                processData: false,
                contentType: false,
                success: function(response) {
                    swal({
                        icon: 'success',
                        title: '{{ __('Add Successfuly') }}',
                    }).then((result) => {
                        // refresh the page
                        $("#add_full_Client").trigger('reset');
                ...
                error: function(response) {
                    ...
                    $.each(errors, function(field, messages) {
                        var input = $('#add_full_Client').find('[name="' + field + '"]');
