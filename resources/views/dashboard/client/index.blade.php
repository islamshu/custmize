...
@section('content')
                            ...
                            <div class="card">
                                ...
                                <div class="card-content collapse show">
                                    <div class="card-body card-dashboard">
                                        ...
                                        @include('dashboard.client.modal')
                                        <table class="table table-striped table-bordered file-export">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>{{ __('Name') }}</th>
                                                    <th>{{ __('Email') }}</th>
                                                    @foreach (App\Models\Extra::where('table', 'client')->where('show_in_table', 'yes')->get() as $item)
                                            ...
                                            <tbody>
                                                @foreach ($clients as $key => $item)
                                                    <tr>
                                                        <th>{{ $key + 1 }}</th>
                                                        <th> <img src="{{ asset('uploads/' . $item->image) }}"
                                                        ...
                                                        @endphp
                                                        @foreach (App\Models\Extra::where('table', 'client')->where('show_in_table', 'yes')->get() as $ex)
                                                            @php
                                                                $name = $ex->name;
                                                            @endphp
    ...
    @include('dashboard.client._extra')
@endsection
