...
$(function () {
  ...
  function updateQuantity(newQuantity) {
    quantity = Math.max(1, newQuantity); // ضمان أن تكون الكمية 1 على الأقل
    numberDisplay.textContent = quantity;
    qtyInput.value = quantity;
    get_price(); // تحديث السعر بناءً على الكمية
}
  function _() {
  ...
  decrease.addEventListener('click', () => {
    if (quantity != 1) {
      ...
      saveDesignAutomatically();  // حفظ تلقائي بعد تعديل الكمية
  ...
  $("#checkout").click(function() {
    ...
    // عند تعديل أي شيء على القماش (مثل الصورة أو النص)
t.on('object:modified', function() {
  saveDesignAutomatically();
});

// حفظ التصميم تلقائيًا
function saveDesignAutomatically() {
  ...
  let material = document.querySelector('.material .active');
  let materialname = material.getAttribute('data-value');
  let qty = $("#qty").val(); // Get the selected quantity
  let price_pice = $('#main_price_input').val();

  // توليد رابط صورة التصميم
  var imageDataUrl = t.toDataURL({ 
      format: "png", 
      multiplier: Math.ceil(1e4 / ((k() * f) / 250)) / 1e4 
  });

  // إزالة البيانات الزائدة من رابط الصورة
  var base64Data = imageDataUrl.replace(/^data:image\/png;base64,/, "");

  // إرسال الصورة والبيانات للسيرفر
  var o = n / (n + 30);
  mergeImages([
      { src: $('#img_shirt').attr('src'), x: 0, y: 0 },
      {
      ...
      $.ajax({
          url: '/save-shirt-image', // الرابط لحفظ الصورة في قاعدة البيانات
          method: 'POST',
          data: {
              product_id: productId,
              shirt_image: shirtImageData,
